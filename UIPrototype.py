...
def next_page():
    ...
    root.after(8500, lambda: stopbar(progress_frame, progress_bar))
...
def stopbar(progress_frame, progress_bar):
    ...
    finish_label = CTkLabel(root,text = "File has been saved at the same directory of this application", font = (("Aldo the Apache", 20)))
    finish_label.place(relx = 0.5, rely = 0.45, anchor = CENTER)
    progress_label.configure(text_color = "#3C9E6D")

    image4 = CTkImage(light_image=Image.open("img14.png"), size=(30,30))
...
imagedel = CTkImage(light_image= Image.open("img13.png"), size = (25,25))
